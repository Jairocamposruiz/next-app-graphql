...
export const ProductSummary = ({ product }: Props) => {
  ...
  const [isVisibleAddedToCart, setIsVisibleAddedToCart] = useState(false);
  ...
  const onAddToCart = () => {
    ...
    setIsVisibleAddedToCart(true);
  };

  useEffect(() => {
    const timeout = setTimeout(() => {
      setIsVisibleAddedToCart(false);
    }, 2000);

    return () => clearTimeout(timeout);
  }, [isVisibleAddedToCart]);
    ...
    <div className={ `w-full max-w-5xl` }>
      ...
      >
        ...
        >
          ...
          <p className={ `text-green-400 font-semibold transition duration-500 mt-2 ${ (isVisibleAddedToCart) ? `opacity-100` : `opacity-0` }` }>
            Added to cart
          </p>
        </div>
      </div>
