...
export function findOne(parent: unknown, args: { id: string }, context: ResolverContext): Promise<Avocado | null> {
  return context.orm.avocado.findUnique({ where: { id: parseInt(args.id, 10) }, include: { attributes: true } });
}
...
export function createAvo(
  ...
  const { description, hardiness, taste, shape, ...rest } = data;
  return context.orm.avocado.create({
    data: {
      ...rest,
      sku: createHash('sha256').update(data.name).digest('hex'),
      attributes: {
        create: {
          description,
          hardiness,
